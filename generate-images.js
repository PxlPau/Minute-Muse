...
const SEASON_THEMES = {
  ...
  tropical: [
    // Indonesia & SE Asia (Bali, Java, Thailand)
    ...
    // Brazil & South America (Amazon, Rio)
...
// --- FIX: Logic corrected below ---
async function fetchImages(seasonKey, period, count = 3) {
  
  // 1. MIX & MATCH LOGIC
  // Use the correct constant names (SEASON_THEMES) and the correct argument (seasonKey)
  const specificSeasonThemes = SEASON_THEMES[seasonKey];
  const randomS = pick(specificSeasonThemes);
    
  // 2. Get random theme from PERIOD
  const specificPeriodThemes = PERIOD_THEMES[period];
  const randomP = pick(specificPeriodThemes);

  // --- COMBINE FOR QUERY ---
  // Example: "tropical monsoon rain misty mountain peak"
  ...
  try {
    ...
    if (!res.ok) {
      // Retry if too specific
      if(res.status === 404) return await fetchFallback(seasonKey, count); 
...
async function fetchFallback(seasonKey, count) {
  ...
  // Fallback to just searching the season name (e.g., "winter")
